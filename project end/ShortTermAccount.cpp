...
long long ShortTermAccount::calculateInterest()
{
          QDateTime currentDate;
          currentDate = QDateTime::currentDateTime();
          if( getCalDateOfInterest().daysTo(currentDate) >= 30){
                int monthNum = getOpenAccDate().daysTo(currentDate) / 30;
           long long interest=0;
           for(int i =0; i < monthNum;i++)
                interest= ( (getBalance() * (getInterest())))+interest; // update balance


              setCalDateOfInterest(getCalDateOfInterest().addMonths(monthNum));
              //har dafe tarikh sooddahi update mishavad va montaghel mishavad be n mah bad
               return interest;
          }
          return 0;
...
void ShortTermAccount::transferMoneyViaAccount(QString accNum, int _index, QString _nCode, long long int amount, QString _toaccountNum)
{
